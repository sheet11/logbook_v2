...
<html>
	...
	<br />
	<?php
	$query = mysql_query("SELECT * FROM tb_logbook, `tb_daftar_skp`, tb_detail_skp WHERE tb_logbook.status='Sudah di Verifikasi' AND tb_logbook.nip='$_GET[pegawai]' AND month(bulan) = '$_GET[bulan]' AND year(bulan) = '$_GET[tahun]' AND tb_logbook.id_daftar_skp=tb_daftar_skp.id_daftar_skp AND tb_logbook.id_detail_skp=tb_detail_skp.id_detail_skp order by id_logbook asc");
