    ...
    <div class="container-fluid" style="margin:30px;">	
		<table width="100%" class="table table-bordered">
			...
			<tr>
				...
				<td colspan="2"><input type="text" id="tgls" name="tanggal" value="<?php echo $_GET['tanggal']; ?>" required="yes" class="form-control"> </td>
			...
			<tr>
				...
				<td colspan="4">
					...
					<select name="skp" id="daftar" class="form-control">
						...
						<?php
							$daftar = mysql_query("select * from tb_daftar_skp_dosen where nip='$_SESSION[nip]' AND month(bulan)='$bulan' AND year(bulan)='$tahun'  ORDER BY id_daftar_skp ASC");
			...
			<tr>
				...
				<td><input type="text" name="jumlah_kegiatan_skp" class="form-control" required></td>
		...
		<?php
			include"../config/koneksi.php";
				...
				if(isset($_POST['proses'])){
					echo"<script>alert('Proses Tanggal Berhasil');window.location='11_tambah_logbook.php?tanggal=$_POST[tanggal]'</script>"; 
					}
					
				if(isset($_POST['submit'])){
						$arr1 = explode(":", $_POST['waktu']);
						$menit1 = $arr1[0]*60;
						$jam1 = $arr1[1];
						$tjam1 = $menit1 + $jam1;
					$query=mysql_query("insert into tb_logbook_dosen(tanggal_logbook, jumlah_menit, total_menit, keterangan, jumlah_kegiatan_skp, output_kegiatan_skp, jumlah_kegiatan, output_kegiatan, status, nip, id_daftar_skp, id_detail_skp)
									values('$_POST[tanggal_logbook]','$_POST[waktu]', '$tjam1','$_POST[keterangan]','$_POST[jumlah_kegiatan_skp]','$_POST[output_kegiatan_skp]','$_POST[jumlah_kegiatan]','$_POST[output_kegiatan]','Belum di Nilai', '$_SESSION[nip]', '$_POST[skp]', '$_POST[detail]')");
						if($query){
							echo"<script>alert('Data Berhasil di Simpan');window.location='11_tambah_logbook.php'</script>";
						}
						else
						{
