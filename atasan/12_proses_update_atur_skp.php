...
<?php
	...
	if(empty($id))
	{
		...
		if($query) 
		{ 
			echo "<script>alert('Pengaturan SKP berhasil di atur');window.location='12_lihat_atur_skp.php?pegawai=$pegawai&bulan=$bulan&tahun=$tahun'</script>";
		}
		else 
		{
			echo "<script>alert('Pengaturan SKP gagal di atur');window.location='12_lihat_atur_skp.php?pegawai=$pegawai&bulan=$bulan&tahun=$tahun'</script>";
		}
	}
	elseif(!empty($id))
	{
