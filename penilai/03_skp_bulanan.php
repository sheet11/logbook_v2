	    ...
	    <form method="post" action="" enctype="multipart/form-data">    
	        <table width="100%" class= "table table-bordered" style="width:50%;">    
				...
				<tr>
					...
					{
						echo "<td><button class='btn btn-success btn-xs' title='SKP Terkunci' disabled ><i class='glyphicon glyphicon-lock'></i></button></td>";
					}
					else
					{
				...
				while($qry=mysql_fetch_array($qr)){
					$target = $target+$qry['target_kegiatan'];
					$query11 = mysql_query("SELECT * FROM tb_pengaturan_skp WHERE month(tanggal_pengaturan_skp) = '$_GET[bulan]' AND year(tanggal_pengaturan_skp) = '$_GET[tahun]' AND nip_pegawai='$_SESSION[nip]'");
					$dapat = mysql_fetch_array($query11);
				echo"
