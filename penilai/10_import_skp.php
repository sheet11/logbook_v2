...
<b>Perhatian:</b>
						<ol>
							...
							<li>Mohon untuk tanggal di isi tanggal 01</li>
						</ol>
					...
					elseif($dapat['status_pengaturan_skp'] == 'Y' OR empty($dapat))
					{
						...
						if($upload)
						{
							...
							for($i=2;$i<=$arrayCount;$i++)
							{
								...
								$namaSkp = trim($allDataInSheet[$i]["B"]);
								$target = trim($allDataInSheet[$i]["C"]);
								$outputKegiatan = trim($allDataInSheet[$i]["D"]);
								$mutu = trim($allDataInSheet[$i]["E"]);
								$nip = $_SESSION['nip'];
								$insertTable= mysql_query("insert into tb_daftar_skp (bulan, nama_skp, target_kegiatan, output_kegiatan, mutu, nip, status) VALUES ('".$realTanggal."', '".$namaSkp."', '".$targetKegiatan."', '".$outputKegiatan."', '".$mutu."', '".$nip."', 'Belum Di Nilai');");
								if($insertTable)
								{
