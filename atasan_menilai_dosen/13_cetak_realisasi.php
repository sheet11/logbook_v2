...
<html>
	...
	$query = mysql_query("SELECT * FROM tb_daftar_skp_dosen, `tb_daftar_skp_dosen`, tb_detail_skp_dosen WHERE tb_daftar_skp_dosen.status='Sudah di Verifikasi' AND tb_daftar_skp_dosen.nip='$_GET[pegawai]' AND month(bulan) = '$_GET[bulan]' AND year(bulan) = '$_GET[tahun]' AND tb_daftar_skp_dosen.id_daftar_skp=tb_daftar_skp_dosen.id_daftar_skp AND tb_daftar_skp_dosen.id_detail_skp=tb_detail_skp_dosen.id_detail_skp order by id_logbook asc");
	...
	$qr = mysql_query("SELECT * FROM `tb_daftar_skp_dosen` as s, tb_pegawai as p WHERE s.nip_penilai=p.nip AND id_daftar_skp='$_GET[id_daftar_skp]'");
				...
				$que=mysql_query("SELECT * FROM `tb_detail_skp_dosen` WHERE id_daftar_skp='$a[id_daftar_skp]' order by id_detail_skp asc");				
